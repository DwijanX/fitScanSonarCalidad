...
export default function InfoScan() {
  ...
  const calorias = alimentos.reduce(
    (acc, current) => acc + current.calorias,
    0
  );
    ...
    <View style={styles.mainContainer}>
      ...
      <View style={styles.imageContainer}>
        {image ? (
          <View>
            <Image
              testID="info-scan-image"
              source={{ uri: image }}
              style={{ width: 200, height: 200, marginLeft: "22%" }}
            />
          </View>
        ) : (
          <Text> null</Text>
        ...
        <View style={[styles.ingredientesBox]}>
          {alimentos.length ? (
            <Text style={styles.foodText}>
              {alimentos
                .map((item) => `${item.nombre}: ${item.calorias}`)
                .join("\n")}
            </Text>
          ) : (
